...
methods = c("decorrelate", "GIW-EB (k=100)", "Schafer-Strimmer", "Pseudoinverse", "Ledoit-Wolf", "OAS", "Touloumis", "lambda = 0.1", "lambda = 1e-4")
# methods = c("decorrelate", "GIW-EB (k=100)", "Schafer-Strimmer", "lambda = 0.1", "lambda = 1e-4")
...
res = mclapply( methods, function(method){
  tm <- system.time({
    ...
    res = lapply( 1:22, function(chrom){
      ...
      # run imputation
      if( method == "GIW-EB (k=100)"){
        res = run_imputez(df_chrom[-idx,], gds, window, flankWidth, method="decorrelate", k=100)
...
# maf vs error
breaks = seq(0.01, .5, length.out=50)
fig = df_res %>%
  filter(maf > MAF) %>%
  mutate(cat = cut(maf, breaks)) %>%
  group_by(method, cat) %>%
  summarize(rMSE = sqrt(mean((z.stat - z)^2))) %>%
  mutate(cat = as.numeric(cat)) %>%
  ggplot(aes(cat, rMSE, color=method)) +
    geom_point() +
    geom_smooth() +
    theme_classic() +
    theme(aspect.ratio=1, legend.position="none") +
    scale_color_manual(values = colMethods) +
    scale_y_continuous(limits=c(NA, 0.24)) 
