...
methods = c("decorrelate", "GIW-EB (k=100)", "Schafer-Strimmer", "Pseudoinverse", "lambda = 0.01", "lambda = 1e-4") #, "Ledoit-Wolf",  "OAS", "Touloumis"

# for each method
res = mclapply( methods, function(method){
  tm <- system.time({
    # for each chrom
    res = lapply( 22, function(chrom){
      ...
      if( method == "GIW-EB (k=100)"){
        ...
        res = run_imputez(df_chrom[-idx,], gds, window, flankWidth, method="S", lambda=0.01)
      }else if( method == "lambda = 1e-4"){        
        res = run_imputez(df_chrom[-idx,], gds, window, flankWidth, method="S", lambda=1e-4)
      }else{      
